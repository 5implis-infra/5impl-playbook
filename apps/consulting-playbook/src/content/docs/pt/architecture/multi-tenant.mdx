...
description: Isolamento de workspaces, fronteiras de dados e responsabilidades por tenant no sistema 5impl.is.
---
...
│ Support Dept    │  │ cliente         │     │                      │
│ Financial Dept  │  │                 │     │ Instância Directus   │
│                 │  │ Directus        │     │ (dados da igreja)    │
│ Directus 5impl  │  │ (produto deles) │     │                      │
│ (CRM + Config)  │  │                 │     │ Hermes profile       │
└─────────────────┘  └─────────────────┘     └──────────────────────┘
        ▲
        │
  TODOS os contratos,
...
| `directus.client-a.com` | Dados do produto do cliente A | Uma instância por cliente de consultoria |
...
**Tag obrigatória:** Cada agente injeta `X-LiteLLM-Tag: {department}.{agent_id}` em toda chamada (ex: `editorial.content_writer`).
...
| `church-{slug}` | Automações operacionais da Igreja | `SubscriptionProvisioner` |
...
| `client-{slug}` | SMTP do cliente | `*@cliente.com` | `WorkspaceProvisioner` |
| `church-{slug}` | SMTP da igreja (opcional) | Configura no onboarding | `OnboardingOrchestrator` |
...
<MermaidDiagram code={`sequenceDiagram
  ...
  WP->>HM: enviar email boas-vindas ao cliente`} caption="Fluxo de criação de tenant — Consultoria" />
...
<MermaidDiagram code={`sequenceDiagram
  ...
  SP->>ZR: enviar WhatsApp boas-vindas ao líder
  SP-->>SP: disparar subscription_activated → OnboardingOrchestrator`} caption="Fluxo de criação de tenant — Church SaaS" />
