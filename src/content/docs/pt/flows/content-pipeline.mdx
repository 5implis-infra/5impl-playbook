...
description: Fluxo completo — Editorial → Aprovação → Publicação → Social → Newsletter com diagrama de sequência.
---
...
<MermaidDiagram code={`sequenceDiagram
  ...
  N8->>MR: editorial_schedule_triggered
  MR->>CW: research_completed
  CW->>SEO: article_written
  SEO->>DP: article_seo_ready
  DP->>ID: draft_created
  ID->>EG: assets_ready
  EG->>S: Issue HITL: "Post pronto para revisão"
  S->>EG: aprova
  ...
  and Newsletter
    NE->>ND: newsletter_ready
    ND->>HM: envia em lote por vertical
  end`} caption="Pipeline de conteúdo — do planejamento à publicação" />
